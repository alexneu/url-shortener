...
RSpec.describe Url, type: :model do
  ...
  describe '#new' do
    it "is valid with a slug and a valid original_url" do
      url = Url.new(slug: 'slug1', original_url: 'http://www.goldbelly.com')
      expect(url).to be_valid
      url.destroy
    end

    it "is not valid without a slug" do
      url = Url.new(slug: nil, original_url: 'http://www.goldbelly.com')
      expect(url).to_not be_valid
    end

    it "is not valid with a duplicate slug" do
      url = Url.create(slug: 'slug1', original_url: 'http://www.goldbelly.com')
      url2 = Url.create(slug: 'slug1', original_url: 'http://www.goldbelly.com')
      expect(url2).to_not be_valid
    end

    it "is not valid with a slug longer than 32 characters" do
      url = Url.new(slug: 'abcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyz', original_url: 'http://www.goldbelly.com')
      expect(url).to_not be_valid
    end
    ...
    it "accepts an optional expiration time in the future" do
      url = Url.new(slug: 'slug1', original_url: 'http://www.goldbelly.com', expiration: Time.now + 1.day)
      expect(url).to be_valid
    end
    ...
    it "accepts an optional user_id" do
      url = Url.new(slug: 'slug1', original_url: 'http://www.goldbelly.com', expiration: Time.now + 1.day, user_id: '123')
  ...
  describe '#prep_random_slug' do
    it 'generates a random 6 character slug' do
      params = { original_url: 'http://www.goldbelly.com' }
      Url.prep_random_slug(params)
      expect(params[:slug]).to eq '8H8Okv'
    end
  end

  describe '#generate_slug' do
    it 'generates a random 6 character slug for the user' do
      slug = Url.generate_slug('http://www.goldbelly.com')
      expect(slug).to eq '8H8Okv'
    end
  end

  describe '#generate_slug' do
    it 'generates a unique random 6 character slug for each original url' do
      slug1 = Url.generate_slug('http://www.goldbelly.com')
      slug2 = Url.generate_slug('http://www.google.com')
  ...
  describe '#generate_slug' do
    it 'generates a unique random 6 character slug for same url if time is different' do
      ...
      slug2 = Url.generate_slug('http://www.goldbelly.com')
      expect(slug1).to_not eq slug2
    end
  end
end
